...
<button type="button" onclick="abrirSidebarAluno()"
...
<div id="overlaySidebarAluno" onclick="fecharSidebarAluno()" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-[9998] hidden lg:hidden"></div>

<aside id="sidebarAluno"
    class="sidebar-aluno group/sidebar fixed lg:sticky top-0 left-0 z-[9999] w-72 lg:w-72 min-h-screen h-screen bg-[#F8FBF8] dark:bg-[#071811] text-[#003C2F] dark:text-[#EAF5EF] border-r border-[#DCE7DE] dark:border-white/10 px-5 py-5 flex flex-col justify-between transform -translate-x-full lg:translate-x-0 transition-all duration-300 ease-in-out shadow-2xl lg:shadow-none overflow-y-auto overflow-x-hidden"
    data-collapsed="false">

    <div class="min-w-0">
        <div class="lg:hidden flex justify-end mb-4">
            <button type="button" onclick="fecharSidebarAluno()" class="bg-[#EAF5EF] dark:bg-white/10 hover:bg-[#DCE7DE] dark:hover:bg-white/15 text-[#003C2F] dark:text-white p-2 rounded-xl transition" aria-label="Fechar menu">
        ...
        <div class="flex items-center gap-3 mb-8 px-1">
            ...
            <div class="sidebar-label min-w-0 transition-all duration-300">
                ...
                <p class="text-xs font-bold text-[#60756B] dark:text-[#9DB7AA] truncate">Área do aluno</p>
            </div>
        </div>

        <div class="hidden lg:flex mb-5">
            <button type="button" onclick="alternarSidebarAluno()"
                ...
                <svg id="iconeRecolherAluno" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <nav class="space-y-7 text-sm font-bold">
                ...
                <div class="space-y-2">
                    @foreach($itensAluno as $item)
                        @if(!empty($item['button']))
                            <button type="button" onclick="{{ $item['onclick'] }}" title="{{ $item['titulo'] }}"
                                class="sidebar-item w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition text-left {{ $item['ativo'] ? 'bg-[#004D3A] text-white shadow-lg shadow-[#004D3A]/15' : 'text-[#3F5D51] dark:text-[#D5E7DD] hover:bg-[#EAF5EF] dark:hover:bg-white/10' }}">
                        ...
                        @else
                            <a href="{{ $item['url'] }}" onclick="{{ $item['onclick'] }}" title="{{ $item['titulo'] }}"
            ...
            @if($ehSuperAdmin)
                <div>
                    ...
                    <a href="{{ route('dashboard.professor') }}" onclick="fecharSidebarAluno()" title="Painel Admin"
                       class="sidebar-item flex items-center gap-3 px-4 py-3 rounded-2xl transition bg-yellow-50 dark:bg-yellow-400/10 text-yellow-800 dark:text-yellow-200 border border-yellow-200 dark:border-yellow-300/20 hover:bg-yellow-100 dark:hover:bg-yellow-400/20">
                        <span class="sidebar-icon w-6 h-6 flex items-center justify-center shrink-0">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7" d="M10.5 6h9.75M10.5 12h9.75M10.5 18h9.75M3.75 6h.008v.008H3.75V6zm0 6h.008v.008H3.75V12zm0 6h.008v.008H3.75V18z"/>
                            </svg>
                        </span>
                        <span class="sidebar-label truncate">Painel Admin</span>
                    </a>
                </div>
            @endif
        </nav>
    ...
    <div class="mt-8 space-y-3">
        <div class="sidebar-profile flex items-center gap-3 px-3 py-3 rounded-3xl bg-white dark:bg-white/10 border border-[#DCE7DE] dark:border-white/10 shadow-sm">
            ...
            <div class="sidebar-label min-w-0">
                <p class="text-sm font-extrabold truncate">{{ $usuarioLogado->name ?? 'Aluno' }}</p>
                <p class="text-xs font-bold text-[#60756B] dark:text-[#9DB7AA] truncate">{{ ucfirst($usuarioLogado->tipo ?? 'aluno') }}</p>
            </div>
        </div>

        <button type="button" onclick="abrirModalSairAluno()" title="Sair"
...
<div id="modalSenhaProvaFinalSidebar" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-[10000] px-4">
    <div class="bg-white dark:bg-[#071811] w-full max-w-sm rounded-3xl shadow-2xl p-6 text-center border border-yellow-100 dark:border-yellow-300/20">
        <div class="w-16 h-16 mx-auto rounded-full bg-yellow-100 dark:bg-yellow-400/10 text-yellow-700 dark:text-yellow-200 flex items-center justify-center mb-4">
            <span class="text-2xl">🔐</span>
        </div>

        <h2 class="text-xl font-extrabold text-gray-800 dark:text-white mb-2">Acesso de teste</h2>
        <p class="text-sm text-gray-500 dark:text-[#9DB7AA] mb-5 leading-relaxed">
            Digite a senha temporária para liberar a prova final apenas para teste.
        </p>

        <input type="password" id="senhaProvaFinalSidebar" placeholder="Digite a senha"
            class="w-full px-4 py-3 rounded-2xl border border-yellow-200 dark:border-yellow-300/20 bg-yellow-50 dark:bg-yellow-400/10 text-gray-800 dark:text-white text-center font-bold focus:outline-none focus:ring-2 focus:ring-yellow-400 mb-3">

        <p id="erroSenhaProvaFinalSidebar" class="hidden text-sm text-red-600 dark:text-red-300 font-bold mb-4">
            Senha incorreta. Tente novamente.
        </p>

        <div class="flex gap-3">
            <button type="button" onclick="fecharModalSenhaProvaFinalSidebar()" class="w-1/2 px-4 py-3 rounded-2xl bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-white font-bold hover:bg-gray-200 dark:hover:bg-white/15 transition">
                Cancelar
            </button>

            <button type="button" onclick="validarSenhaProvaFinalSidebar()" class="w-1/2 px-4 py-3 rounded-2xl bg-[#004D3A] text-white font-bold hover:bg-[#003C2F] transition">
                Entrar
            </button>
        </div>

        <a href="{{ route('prova.final') }}" class="block mt-4 text-xs text-gray-500 dark:text-[#9DB7AA] hover:text-[#004D3A] dark:hover:text-[#8EF3C5] font-bold">
            Entrar sem senha e verificar liberação normal
        </a>
    </div>
</div>

<div id="modalSairAluno" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-[10000] px-4">
    <div class="bg-white dark:bg-[#071811] w-full max-w-sm rounded-3xl shadow-2xl p-6 text-center border border-[#E3EBE4] dark:border-white/10">
        ...
        <div class="flex gap-3">
            <button type="button" onclick="fecharModalSairAluno()" class="w-1/2 px-4 py-3 rounded-2xl bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-white font-bold hover:bg-gray-200 dark:hover:bg-white/15 transition">
                Cancelar
            </button>
